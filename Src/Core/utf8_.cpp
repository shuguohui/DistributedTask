	...
	std::string Unicode2Utf8(const std::wstring& uniStr)
{
	...
	char szLocalBuffer[LOCAL_BUFFER_SIZE];
	char* sValue = szLocalBuffer;
	void* pHeapBuffer = NULL;
	if(utf8Len > LOCAL_BUFFER_SIZE)
	{
		pHeapBuffer = malloc(utf8Len);
		sValue = (char* )pHeapBuffer;
	}
	if(!sValue)
		return std::string();

	int ret = ut_utf8_from_unicode(uniStr.c_str(),uniStr.length(),sValue,utf8Len);
	std::string strRet(sValue);
	if(pHeapBuffer)
		free(pHeapBuffer);
	return strRet;
}
	...
	std::wstring Utf82Unicode(const std::string& utf8Str)
{
	...
	wchar_t szLocalBuffer[LOCAL_BUFFER_SIZE / sizeof(wchar_t)];
	wchar_t* wValue = szLocalBuffer;
	void* pHeapBuffer = NULL;
	if(wideLen > LOCAL_BUFFER_SIZE / sizeof(wchar_t))
	{
		pHeapBuffer = malloc(wideLen * sizeof(wchar_t));
		wValue = (wchar_t* )pHeapBuffer;
	}
	if(!wValue)
		return std::wstring();

	int ret = ut_utf8_to_unicode(utf8Str.c_str(),utf8Str.length(),wValue,wideLen);
	std::wstring strRet(wValue);
	if(pHeapBuffer)
		free(pHeapBuffer);
	return strRet;
}
